...
자바스크립트 함수는 실행되기 전에 함수 안에 필요한 변수값들을 모두 모아서 **유효 범위의 최상단에 선언**한다.
...
- 실제로 코드가 끌어올려지는 것은 아니며, 자바스크립트 파서가 내부적으로 끌어올려서 처리하는 것이다.
- 실제 메모리에서는 변화가 없다.
...
### 변수 호이스팅 예시 (var vs let / const)

```javascript
console.log("hello");

var myName = "JEEE"; // var 변수
let myName2 = "Jeee"; // let 변수
```
...
console.log("hello");
myName = "JEEE"; // "할당"
let myName2 = "Jeee"; // [Hoisting] 발생 x
```

### 함수 호이스팅 예시 (선언문 vs 표현식)
...
```javascript
// JS Parser 내부의 호이스팅(Hoisting) 결과 - 위와 동일
...
[함수선언문과 함수표현식의 차이](https://gmlwjd9405.github.io/2019/04/20/function-declaration-vs-function-expression.html)
...
function printName(firstName) {
  ...
  var resultl; // [Hoisting] var 변수값 선언
...
// TypeError
function printName(firstName) {
  console.log(inner); // "undefined" - 선언은 되어 있지만 값이 할당되지 않은 상태
...
printName에서 "inner is not defined" 이라고 오류가 나오지 않고, "inner is not a function" 이라고 TypeError가 나오는 이유는 무엇일까?  
바로 printName이 실행되는 순간 호이스팅에 의해 inner는 "undefined"로 지정이 되기 때문이다.  
inner가 undefined라는 것은 아직 함수로 인식되지 않고 있음을 의미한다.

**#3. 함수표현식의 선언이 호출보다 아래에 있는 경우 (let / const 변수)**
...
함수표현식보다 함수선언문을 더 자주 사용하지만, 어떤 코딩컨벤션에는 함수표현식을 권장하기도 한다.

- 어떤 컨벤션을 갖던지, 한가지만 정해서 사용하는 것이 좋다.
...
> - 따라서 아직까지는 var가 어떻게 동작하는지 이해하고 있어야 한다.
